...
제품 범위 판단은 README보다 이 문서를 우선한다.

## 목표
- 해시 테이블 기반 인메모리 `Mini Redis`를 구현한다.
- FastAPI 기반 HTTP JSON API를 제공한다.
- TTL, DB 기반 데이터 캐싱, 단일 스레드 직렬 처리 시연까지 포함한다.
...
- 캐시 데모는 첫 조회와 재조회에서 origin/cache 흐름을 구분해 보여줘야 한다.
...
- 캐시 시나리오 기본 TTL은 `15초`다.
- 동시성 시연은 하나의 shared command executor thread에서 모든 명령을 순차 처리한다.
- 좌석 예약 시연 기본값은 `seatLimit = 50`, `requestCount = 100`이다.
- 좌석 예약 시연에서 초과 요청은 큐 미진입이 아니라 `soldOut` 실패로 종료한다.

## Definition of Done
- KV API와 TTL API가 계약대로 동작한다.
- 캐시 시나리오가 DB origin과 cache hit를 구분해 보여준다.
- 좌석 예약 동시성 데모가 순차 처리 결과를 timeline으로 보여준다.
...
- `AC-13`: 좌석 예약 timeline은 `requestCount` 길이와 같은 결과를 포함한다.
- `AC-14`: 성공 항목은 `seatNumber`를 갖고 실패 항목은 `seatNumber = null`이다.
- `AC-15`: README와 데모 페이지가 새 동시성 시연 흐름을 설명한다.
