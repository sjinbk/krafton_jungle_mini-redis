...
## 뉴스 캐싱 요청 흐름
1. 클라이언트가 `GET /demo/external-cache?topic=...`를 호출한다.
2. API 계층이 `topic`을 검증하고 서비스 계층에 전달한다.
3. 서비스 계층이 캐시 키를 계산한 뒤 저장소에서 조회한다.
4. 캐시 hit면 남은 TTL과 함께 캐시 데이터를 반환한다.
5. 캐시 miss면 업스트림 뉴스 API를 호출한다.
6. 업스트림 결과가 있으면 저장소에 TTL과 함께 저장한 뒤 응답한다.
7. 업스트림 결과가 비어 있으면 빈 배열 응답만 반환한다.
