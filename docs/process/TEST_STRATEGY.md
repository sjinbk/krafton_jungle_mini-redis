...
- 성능 비교:
  - 동일한 입력으로 no-cache와 cache hit를 한 번 이상 비교한다
...
- 뉴스 헤드라인 캐싱 시나리오에서 캐시 hit를 확인할 수 있다
- TTL 만료 후 뉴스 API 재호출이 발생한다
- 지원하지 않는 `topic` 요청은 `400`과 `INVALID_TOPIC`을 반환한다
- 유효한 `topic`인데 업스트림 결과가 비면 `200`과 빈 배열을 반환한다

최소 세트:
- `POST /kv` -> `GET /kv/{key}`
- `POST /kv/{key}/expire` 후 만료 전/후 조회
- `GET /demo/external-cache?topic=ai` 첫 호출과 재호출 비교
- invalid topic 응답
