...
class BlockingObservedStore(InMemoryStore):
    ...
    def _record(self, phase: str, key: str) -> None:
        with self._events_lock:
            self._events.append(TimelineEvent(phase=phase, key=key, timestamp=perf_counter()))


class BlockingOriginRepository:
    def __init__(self, items_by_key: dict[str, list[dict[str, str]]], blocked_keys: set[str]) -> None:
...
def _build_test_app(*, store: InMemoryStore, origin_repository: BlockingOriginRepository):
    return create_app(
        settings=Settings(
...
def _start_request(client: TestClient, method: str, path: str, **kwargs: Any) -> tuple[Thread, dict[str, Any]]:
...
def test_kv_create_get_delete_flow(integration_app) -> None:
    with TestClient(integration_app) as client:
        create_response = client.post("/kv", json={"key": "sample", "value": {"status": "ready"}})
        get_response = client.get("/kv/sample")
        delete_response = client.delete("/kv/sample")
        missing_response = client.get("/kv/sample")

    assert create_response.status_code == 201
    assert create_response.json()["data"] == {"key": "sample", "value": {"status": "ready"}}
    assert get_response.status_code == 200
    assert delete_response.status_code == 200
    assert delete_response.json()["data"] == {"key": "sample", "deleted": True}
...
def test_kv_expire_and_ttl_flow(integration_app) -> None:
    ...
    assert ttl_response.json()["data"] == {"key": "ttl-key", "hasTtl": True, "ttlSecondsRemaining": 5}
...
def test_cache_compare_performance_api_accepts_custom_input(integration_app) -> None:
    with TestClient(integration_app) as client:
        response = client.post("/demo/performance/cache-compare", json={"key": "alpha", "iterations": 3})
...
def test_seat_reservation_api_returns_expected_default_result(integration_app) -> None:
    with TestClient(integration_app) as client:
        response = client.post("/demo/concurrency/seat-reservation")

    assert response.status_code == 200
    data = response.json()["data"]
    assert data["scenario"] == "seatReservation"
    assert data["seatLimit"] == 50
    assert data["requestCount"] == 100
    assert data["reservedCount"] == 50
    assert data["soldOutCount"] == 50
    assert data["serialExecutor"] is True
    assert len(data["timeline"]) == 100
    assert sum(1 for item in data["timeline"] if item["statusCode"] == 200) == 50
    assert sum(1 for item in data["timeline"] if item["statusCode"] == 409) == 50


def test_seat_reservation_api_accepts_custom_input(integration_app) -> None:
    with TestClient(integration_app) as client:
        response = client.post("/demo/concurrency/seat-reservation", json={"seatLimit": 3, "requestCount": 5})

    assert response.status_code == 200
    data = response.json()["data"]
    assert data["seatLimit"] == 3
    assert data["requestCount"] == 5
    assert data["reservedCount"] == 3
    assert data["soldOutCount"] == 2
    assert len(data["timeline"]) == 5


def test_seat_reservation_api_returns_expected_validation_errors(integration_app) -> None:
    with TestClient(integration_app) as client:
        invalid_seat_limit = client.post("/demo/concurrency/seat-reservation", json={"seatLimit": 0, "requestCount": 5})
        invalid_request_count = client.post("/demo/concurrency/seat-reservation", json={"seatLimit": 5, "requestCount": 0})

    assert invalid_seat_limit.status_code == 400
    assert invalid_seat_limit.json()["error"]["code"] == "INVALID_SEAT_LIMIT"
    assert invalid_request_count.status_code == 400
    assert invalid_request_count.json()["error"]["code"] == "INVALID_REQUEST_COUNT"


def test_kv_and_demo_cache_requests_share_the_same_serial_executor() -> None:
    store = BlockingObservedStore(set())
    origin_repository = BlockingOriginRepository({"sample": [{"id": "sample-1", "value": "payload"}]}, {"sample"})
    app = _build_test_app(store=store, origin_repository=origin_repository)

    with TestClient(app) as client:
        first_thread, first_outcome = _start_request(client, "GET", "/demo/data-cache", params={"key": "sample"})
        assert origin_repository.wait_until_entered("sample")
        second_thread, second_outcome = _start_request(client, "POST", "/kv", json={"key": "alpha", "value": "one"})
        sleep(0.1)
